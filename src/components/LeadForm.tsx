...
export function LeadForm({ pharmacy, onComplete, onBack }: LeadFormProps) {
  ...
  const handlePhoneVerificationComplete = async (verified: boolean) => {
    ...
    try {
      ...
      try {
        ...
        console.log('Dados enviados para RD Station:', rdStationData);
        
        // Aqui seria a integração real com RD Station
  ...
  if (success) {
    return (
      <ThankYouPage 
        customerName={leadData.customer}
        pharmacy={leadData.pharmacy}
        funcionario={leadData.funcionario}
      />
    );
  }
